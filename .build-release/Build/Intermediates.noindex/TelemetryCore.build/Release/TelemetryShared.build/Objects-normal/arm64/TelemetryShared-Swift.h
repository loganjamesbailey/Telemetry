...
#ifndef TELEMETRYSHARED_SWIFT_H
#define TELEMETRYSHARED_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="TelemetryShared",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// applied. All parameters are primitives or <code>Data</code> so there are no
/// NSSecureCoding class-allowlist pitfalls.
SWIFT_PROTOCOL("_TtP15TelemetryShared23TelemetryHelperProtocol_")
@protocol TelemetryHelperProtocol
...
- (void)handshakeWithClientProtocolVersion:(NSInteger)clientProtocolVersion reply:(void (^ _Nonnull)(NSString * _Nonnull, NSInteger))reply;
/// Forced-RPM command, lease-scoped. The daemon clamps to the fan’s real
/// [min, max] and reports what it actually applied; <code>leaseSeconds</code> is
/// capped at <code>HelperConstants.maxLeaseSeconds</code>. When the lease expires
/// without renewal the daemon restores automatic control on its own.
- (void)setTargetRPMWithFanIndex:(NSInteger)fanIndex rpm:(double)rpm leaseSeconds:(NSInteger)leaseSeconds reply:(void (^ _Nonnull)(NSInteger, double))reply;
/// Extends the current lease without changing the target.
- (void)renewLeaseWithLeaseSeconds:(NSInteger)leaseSeconds reply:(void (^ _Nonnull)(NSInteger))reply;
/// Hands one fan / all fans back to macOS automatic control.
- (void)releaseToAutoWithFanIndex:(NSInteger)fanIndex reply:(void (^ _Nonnull)(NSInteger))reply;
- (void)releaseAllToAutoWithReply:(void (^ _Nonnull)(NSInteger))reply;
/// JSON-encoded <code>HelperStatus</code>.
- (void)statusWithReply:(void (^ _Nonnull)(NSData * _Nonnull))reply;
@end
